...
class Tdigest
{
    ...
    struct tdigest_impl {
        ...
        enum class InsertResult { OK, NEED_COMPRESS };

        InsertResult insert(Values value, Weight weight)
        {
            assert(weight);
            values.emplace_back(value, weight);
            total_weight += weight;
            return (values.size() != values.capacity() ? InsertResult::OK
                                                       : InsertResult::NEED_COMPRESS);
        }

        InsertResult insert(const centroid_t &val)
        {
...
public:
    ...
    void insert(Values value, Weight weight)
    {
        if (buffer.insert(value, weight)
            == tdigest_impl::InsertResult::NEED_COMPRESS) {
            merge();
        }
    }
...
void Tdigest<Values, Weight>::insert(const Tdigest<Values, Weight> &src)
{
    ...
    auto insert_fn = [this](const auto &val) {
        if (buffer.insert(val) == tdigest_impl::InsertResult::NEED_COMPRESS) { merge(); }
    };
